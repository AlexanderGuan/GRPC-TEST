...
func main() {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second) // 创建一个 10 秒的超时时间的上下文
	stream, err := c.BidirectionalStreamingEcho(ctx)                         // 调用服务端的双向流 RPC 方法 BidirectionalStreamingEcho()，返回表示双向流的变量 stream
	if err != nil {
