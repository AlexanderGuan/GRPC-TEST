...
func main() {
	...
	conn, err := grpc.Dial(*addr, grpc.WithInsecure()) // To call service methods, we first need to create a gRPC channel to communicate with server. We create this by passing the server address and port number to grpc.Dial()
	if err != nil {
	...
	if err != nil {
		...
		if codes.InvalidArgument == errStatus.Code() {
			fmt.Println("You can take specific action on specific error!")
		}
	}
	fmt.Printf("reponse:\n")
	fmt.Printf(" - %q\n", resp.GetMessage())
